package com.cn.android.network;

/**
 * Created by desk 3-2 on 2018/11/19.
 */

public class Constant {
    public static final int GETHTML = 1000;
    public static final int POSTSTRING = 1001;
    public static final int UPLOADFILE = 1002;
    public static final int DOWNLOADFILE = 1003;
    public static final int GETIMAGE = 1004;
    public static final int sendSms = 1005;
    public static final int register = 1006;
    public static final int login = 1007;
    public static final int updateHeadImg = 1008;
    public static final int updateAccount = 1009;
    public static final int updatePhone = 1100;
    public static final int addAddress = 1101;
    public static final int selectAddressByUserid = 1102;
    public static final int deleteAddress = 1103;
    public static final int updateSex = 1104;
    public static final int selectConpostByUserid = 1105;
    public static final int selectAccountByUserid = 1106;
    public static final int addFeedBack = 1107;
    public static final int updateNickname = 1108;
    public static final int addAccountByUserid = 1109;
    public static final int upload = 1110;
    public static final int idcard_front = 1111;
    public static final int idcaard_back = 1112;
    public static final int business_img = 1113;
    public static final int cater_img = 1114;
    public static final int realAppUser = 1115;
    public static final int selectHome = 1116;
    public static final int selectNewShopList = 1117;
    public static final int selectFristTypeList = 1118;
    public static final int selectTypeListByPid = 1119;
    public static final int updateStore = 1120;
    public static final int selectWordsById = 1121;
    public static final int selectMarketingUserById = 1122;
    public static final int selectShopListByShopType = 1123;
    public static final int selectShopsByUserid = 1124;
    public static final int selectShopListByUserid = 1125;
    public static final int deleteShopByUserid = 1126;
    public static final int selecttTeamByUserid = 1127;
    public static final int selectConlectShopsByUserid = 1128;
    public static final int sureConlectShopsByUserid = 1129;
    public static final int addShopEva = 1130;
    public static final int selectOrderListByStatus = 1131;
    public static final int upShopByUserid = 1133;
    public static final int sortShopByUserid = 1134;
    public static final int selectAppUserByUserid = 1135;
    public static final int sureSendOrder = 1136;
    public static final int surePickOrder = 1137;
    public static final int selectOrdersByStatus = 1140;
    public static final int selectShopListBySearch = 1141;
    public static final int saveShopByUserid = 1142;
    public static final int updateHeadImg01 = 1143;
    public static final int selectStoreListByPid = 1143;
    public static final int selectShopByid = 1144;
    public static final int selectShopByID = 1144;

}
