package com.cn.android.widget.AddressPickerLib;
...
public class AddressPickerView extends RelativeLayout implements View.OnClickListener {
    ...
    // 确定字体可以点击时候的颜色
    private int defaultSureCanClickColor = Color.parseColor("#2196F3");
    ...
    private void init(Context context) {
        ...
        // UI
        View rootView = inflate(mContext, R.layout.public_address_picker_view, this);
        // 确定
...
//        }
                ...
                case 1:
                    ...
                    if (mSelectProvice != null) {
                        for (YwpAddressBean.AddressItemBean itemBean : mYwpAddressBean.getCity()) {
                            if (itemBean.getI().equals(mSelectProvice.getP()))
                ...
                case 2:
                    ...
                    if (mSelectProvice != null && mSelectCity != null) {
                        for (YwpAddressBean.AddressItemBean itemBean : mYwpAddressBean.getDistrict()) {
                            if (itemBean.getI().equals(mSelectCity.getP()))
                                mRvData.add(itemBean);
                        }
                    } else {
    ...
    class AddressAdapter extends RecyclerView.Adapter<AddressAdapter.ViewHolder> {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new ViewHolder(LayoutInflater.from(mContext).inflate(R.layout.public_item_address_text, parent, false));
        }
            ...
            switch (tabSelectPosition) {
                case 0:
                    if (mRvData.get(position) != null &&
                            mSelectProvice != null &&
                            mRvData.get(position).getP().equals(mSelectProvice.getP())) {
                        holder.mTitle.setTextColor(defaultSelectedColor);
                    }
                    break;
                case 1:
                    if (mRvData.get(position) != null && mSelectCity != null && mRvData.get(position).getP().equals(mSelectCity.getP())) {
                        holder.mTitle.setTextColor(defaultSelectedColor);
                    }
                    break;
                case 2:
                    if (mRvData.get(position) != null && mSelectDistrict != null && mRvData.get(position).getP().equals(mSelectDistrict.getP())) {
                        holder.mTitle.setTextColor(defaultSelectedColor);
                    }
                    break;
            }
